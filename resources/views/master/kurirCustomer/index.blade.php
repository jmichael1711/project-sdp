            ...
            <div style="overflow-x: auto" class="card-body"><h5 class="card-title">Semua Kurir Customer</h5>
                <table style="min-width: 100%;" class="table table-hover table-striped dataTable dtr-inline" id="tableKendaraan">
                    ...
                    <tbody>
                        ...
                        @foreach ($kurircustomer as $kurcust)
                            <tr onclick='editKurcust("{{$kurcust->id}}")'>
                                <td>{{$kurcust['id']}}</td>
                                <td>{{$kurcust['resi_id']}}</td>
                                <td>{{$kurcust['kurir_customer_id']}}</td>
                                <td>{{$kurcust['berat_barang']}}</td>
                                <td>{{$kurcust['alamat_asal']}}, {{$pesanan['kota_asal']}}}</td>
                                <td>{{$kurcust['alamat_tujuan']}}, {{$pesanan['kota_tujuan']}}}</td>
                                <td>{{$kurcust['nama_pengirim']}}</td>
                                <td>{{$kurcust['nama_penerima']}}</td>
                                @if ($kurcust['status'] == 1)
...
<script>
    ...
    function editKurcust(id){
        window.location.href='/admin/kurir_cusotmer/edit/' + id;
    }
